...
but the \emph{acoustic diffraction limit of the read path}, which caps a realistic
megahertz-addressed pod at ${\sim}10^{5}$--$10^{6}$ bits per cubic millimetre - some
8--15 orders of magnitude below the molecular and structured-water ceilings - while the
...
Read-out is acoustic (\cref{sec:read}), so spatial resolution is bounded by the probe
wavelength. With sound speed $c\approx\qty{1500}{\meter\per\second}$ in water (and
${\sim}\qty{3200}{\meter\per\second}$ in ice), a \qty{50}{\mega\hertz} pulse has
$\lambda\approx\qty{30}{\micro\meter}$ and resolves voxels no finer than
$\lambda/2\approx\qty{15}{\micro\meter}$. Absorption climbs as ${\sim}f^2$, so halving the
voxel by going to ${\sim}\qty{100}{\mega\hertz}$ costs steeply in penetration depth - a hard,
physics-imposed resolution--penetration trade. A \qty{1}{\milli\meter} pod thus contains
${\sim}(1000/15)^3\approx 3\times10^{5}$ resolvable voxels; allotting several bits per voxel
by exploiting phase and amplitude lifts this only to ${\sim}10^{6}$ bits - of order
\qty{100}{\kilo\byte} per cubic-millimetre pod.\footnote{The two-axis scheme of
\cref{sec:biaxial} makes the budget concrete and, deliberately, more conservative: a
\qty{5}{\mega\hertz} axial former ($\lambda_z\approx\qty{300}{\micro\meter}$) yields only
${\sim}6$ freeze-robust discs across a \qty{1}{\milli\meter} pod, and a \qty{50}{\mega\hertz}
transverse field ${\sim}(1000/15)^2\approx 4\times10^{3}$ cells per disc, for
${\sim}2\times10^{4}$ voxels - about an order of magnitude \emph{below} the isotropic
${\sim}3\times10^{5}$ ceiling, the price of trading axial resolution for a coarse,
freeze-robust layer index.}

The verdict follows immediately. The realistic ${\sim}10^{6}$ bits/mm$^3$ sits roughly
\emph{eight} orders of magnitude below the structured-water theory's own
${\sim}1.5\times10^{14}$ bits/mm$^3$, and roughly \emph{thirteen} below the one-bit-per-molecule
ceiling. \emph{You cannot read what you cannot resolve}; the molecular real estate is
irrelevant if the only available probe paints it with a ${\sim}\qty{15}{\micro\meter}$ brush.
...
Spread over the diffraction-limited ${\sim}10^{6}$ bits, that is ${\sim}\qty{4e-7}{\joule}$
per bit (${\sim}400$\,nJ/bit) of heat that must be \emph{removed} - by a refrigerator with
...
energy is therefore ${\sim}10^{4}$--$10^{6}\times$ worse per bit before refrigeration
...
ACPCM - diffraction-limited reality
  & ${\sim}10^{8}$\,B/cm$^3$ (${\sim}10^{6}$ bits/mm$^3$)
  & \textcolor{red!70!black}{\bfseries This work}; set by read wavelength (\cref{sec:diffraction}) \\
\bottomrule
\end{tabularx}
...
resolve: ${\sim}8$ orders of magnitude, with a further ${\sim}5$ to the one-bit-per-molecule
