...
- **Cross-vendor (Gemini/Codex) is OFF unless the user explicitly consents per-target** — sending repo content to external models is blocked by the classifier and by the user's "no third party without consent" rule. octo persona panels (Claude-family, local) are fine.
